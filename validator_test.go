...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid struct passed by pointer",
			args: args{
				v: &struct {
					Len string `validate:"len:20"`
				}{
		...
		{
			name: "valid struct with nested validation",
			args: args{
				v: struct {
					Foo struct {
						Len string `validate:"len:20"`
		...
		{
			name: "valid struct with nested struct without nested validation",
			args: args{
				v: struct {
					Foo struct {
						Len string `validate:"len:20"`
					...
					Foo: struct {
						...
						Len: "abcdefghjklmopqrstvuasdfasdfasdfadsfadsfadsfasdfadsfasdf",
					},
				},
			},
			wantErr: false,
		...
		{
			name: "invalid struct with invalid nested validation",
			args: args{
				v: struct {
					Foo struct {
						Len string `validate:"len:20"`
